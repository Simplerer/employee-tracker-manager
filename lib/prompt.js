...
const budget = async () => {
    const [depoResults] = await db.promise().query('SELECT name FROM departments');
    console.log(depoResults)
    for (let i = 0; i < depoResults.length; i++) {
        let departments = depoResults[i].name;
...
const deleteEmployee = async () => {
    const [empResults] = await db.promise().query(`SELECT CONCAT(first_name, ' ', last_name) AS name FROM employees`);
    
    for (let i = 0; i < empResults.length; i++) {
        let employee = empResults[i].name;
...
const deleteRole = async () => {
    const [roleResults] = await db.promise().query('SELECT title FROM roles');

    for (let i = 0; i < roleResults.length; i++) {
        let role = roleResults[i].title;
        roles.push(role);
    };
...
const deleteDepartment = async () => {
    const [depoResults] = await db.promise().query('SELECT * FROM departments');
