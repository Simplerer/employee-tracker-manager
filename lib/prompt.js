...
const fullRoleTable = `SELECT r.id AS role_id, title, name AS department, salary FROM roles AS r left join departments AS d ON r.department_id = d.id`;
// Simple Query  ------
const showEmployee = `SELECT CONCAT(first_name, ' ', last_name) AS name FROM employees`;
const showDepartments = `SELECT * FROM departments`;
const showRoles = 'SELECT title AS name FROM roles';
...
const budget = async () => {
    const [depoResults] = await db.promise().query(showDepartments);

    fillChoices(depoResults, depo)

    prompt([
        {
            type: 'rawlist',
            name: 'utilized',
            message: 'View total utilized budget of which department?',
            choices: depo
        }
    ])
    .then(({utilized}) => {
        db.query(`SELECT name, SUM(roles.salary) AS salary_budget FROM departments JOIN roles ON roles.department_id = departments.id WHERE name = "${utilized}"`, (err, res) => {
            console.log('\n\n');
            console.table(res, 'Money well spent\n');
            depo = [];
            init();
...
const deleteEmployee = async () => {
    const [empResults] = await db.promise().query('SELECT * FROM employees');
    ...
    .then(({ deletes }) => {
        ...
        let remove = deletes.split(' ').shift();
        console.log('\n\n');
        db.query(`DELETE FROM employees WHERE first_name = "${remove}"`)
        db.query(fullEmpTable, (err, res) => {
            console.log('\n\n');
            console.table(res, 'Employee Deleted\n');
            employees = [];
            init();
        })
    })
...
const deleteRole = async () => {
    const [roleResults] = await db.promise().query(showRoles);

    fillChoices(roleResults, roles);
    ...
    .then(({ deletes }) => {
        console.log('\n\n')
        db.query(`DELETE FROM roles WHERE title = "${deletes}"`)
        db.query(fullRoleTable, (err, res) => {
        console.log('\n\n');
        console.table(res, 'Role Deleted\n');
        roles = [];
        init();
        })
    })
...
const addRole = () => {
    ...
    prompt([
        ...
        .then(({ roleName, roleSalary, roleDepo }) => {
            ...
            db.query(fullRoleTable, (err, results) => {
...
const addDepartment = () => {
    prompt([
        ...
        .then(({ depoName }) => {
            ...
            db.query(showDepartments, (err, results) => {
